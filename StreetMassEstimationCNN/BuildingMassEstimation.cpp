...
namespace bme {
	...
	std::vector<float> estimate(int screen_width, int screen_height, std::vector<vp::VanishingLine> silhouette, boost::shared_ptr<Classifier> classifier, std::vector<boost::shared_ptr<Regression>> regressions, std::vector<cga::Grammar>& grammars, bool automaticRecognition, int& grammarSnippetId, int image_size, bool grayscale, bool centering3D, bool meanSubtraction, float cameraDistanceBase, int xrotMin, int xrotMax, int yrotMin, int yrotMax, int zrotMin, int zrotMax, int fovMin, int fovMax, int xMin, int xMax, int yMin, int yMax, bool tryMultiples, int numMultipleTries, float maxNoise, bool refinement, int maxIters, bool applyTexture) {
		...
		return regress(screen_width, screen_height, silhouette, regressions[grammarSnippetId], grammars[grammarSnippetId], image_size, grayscale, centering3D, meanSubtraction, mean_file, cameraDistanceBase, xrotMin, xrotMax, yrotMin, yrotMax, zrotMin, zrotMax, fovMin, fovMax, xMin, xMax, yMin, yMax, tryMultiples, numMultipleTries, maxNoise, refinement, maxIters, applyTexture);
	}

	/**
	 * 指定されたシルエットに対してClassifierによりgrammar idを特定し、返却する。
	 */
	int classify(int screen_width, int screen_height, std::vector<vp::VanishingLine>& silhouette, boost::shared_ptr<Classifier> classifier) {
	...
	std::vector<float> regress(int screen_width, int screen_height, std::vector<vp::VanishingLine>& silhouette, boost::shared_ptr<Regression> regression, cga::Grammar& grammar, int image_size, bool grayscale, bool centering3D, bool meanSubtraction, const std::string& mean_file, float cameraDistanceBase, int xrotMin, int xrotMax, int yrotMin, int yrotMax, int zrotMin, int zrotMax, int fovMin, int fovMax, int xMin, int xMax, int yMin, int yMax, bool tryMultiples, int numMultipleTries, float maxNoise, bool refinement, int maxIters, bool applyTexture) {
		...
		for (int multipleTryIter = 0; multipleTryIter < (tryMultiples ? numMultipleTries : 1); ++multipleTryIter) {
			...
			double dist = distance(screen_width, screen_height, silhouette, grammar, centering3D, cameraDistanceBase, xrotMin, xrotMax, yrotMin, yrotMax, zrotMin, zrotMax, fovMin, fovMax, xMin, xMax, yMin, yMax, params);
		...
		if (refinement) {
			...
			for (int iter = 0; iter < maxIters; ++iter) {
				...
				double next_dist = distance(screen_width, screen_height, silhouette, grammar, centering3D, cameraDistanceBase, xrotMin, xrotMax, yrotMin, yrotMax, zrotMin, zrotMax, fovMin, fovMax, xMin, xMax, yMin, yMax, next_params);
				
				// execute the coordinate descent
				if (next_dist < dist_threshold) {
					// coordinate descent
					coordinateDescent(next_params, next_dist, params_var, screen_width, screen_height, silhouette, regression, grammar, centering3D, meanSubtraction, cameraDistanceBase, xrotMin, xrotMax, yrotMin, yrotMax, zrotMin, zrotMax, fovMin, fovMax, xMin, xMax, yMin, yMax);
				}
	 ...
	 */
	void coordinateDescent(std::vector<float>& params, double& dist, std::vector<float>& params_var, int screen_width, int screen_height, std::vector<vp::VanishingLine>& silhouette, boost::shared_ptr<Regression> regression, cga::Grammar& grammar, bool centering3D, bool meanSubtraction, float cameraDistanceBase, int xrotMin, int xrotMax, int yrotMin, int yrotMax, int zrotMin, int zrotMax, int fovMin, int fovMax, int xMin, int xMax, int yMin, int yMax) {
		double delta = 0.02;
		...
		for (int iter = 0; iter < 8; ++iter) {
			for (int iter2 = 0; iter2 < 10000; ++iter2) {
				...
				for (int k = 0; k < params.size(); ++k) {
					...
					double dist1 = distance(screen_width, screen_height, silhouette, grammar, centering3D, cameraDistanceBase, xrotMin, xrotMax, yrotMin, yrotMax, zrotMin, zrotMax, fovMin, fovMax, xMin, xMax, yMin, yMax, params1);

					// option 2
					std::vector<float> params2 = params;
					params2[indices[k]] -= sqrt(params_var[indices[k]]) * delta;
					double dist2 = distance(screen_width, screen_height, silhouette, grammar, centering3D, cameraDistanceBase, xrotMin, xrotMax, yrotMin, yrotMax, zrotMin, zrotMax, fovMin, fovMax, xMin, xMax, yMin, yMax, params2);
	...
	*/
	double distance(const std::vector<vp::VanishingLine>& silhouette, const cv::Mat& renderedImg, int screen_width, int screen_height) {
	...
	*/
	/*
	void shiftSilhouette(int shift_x, int shift_y, std::vector<vp::VanishingLine>& silhouette) {
	...
	*/
	/*
	void rotateSilhouette(float theta, const glm::dvec2& center, std::vector<vp::VanishingLine>& silhouette) {
		for (int i = 0; i < silhouette.size(); ++i) {
	...
	* @return			ずれ
	*/
	glm::vec2 getOffset(const std::vector<vp::VanishingLine>& silhouette, int screen_width, int screen_height) {
		// compute the bbox
		glutils::BoundingBox bbox;
		for (auto stroke : silhouette) {
	 ...
	 */
	glm::vec2 getCenter(const std::vector<vp::VanishingLine>& silhouette) {
		// compute the bbox
		glutils::BoundingBox bbox;
		for (auto stroke : silhouette) {
			bbox.addPoint(stroke.start);
	...
	* シルエットの全長を返却する。
	*/
	float silhouetteLength(const std::vector<vp::VanishingLine>& silhouette) {
